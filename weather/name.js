   ...
   async function getweatherdata(){
    ...
    function  showweather(data){
       // document.getElementById("container").innerHTML="" ;
        ...
        ctnm.textContent=`City Name: ${data.city.name}`
        let p8=document.createElement("div")
        p8.textContent=data.list[0].dt_txt
        ctnm.append(p8)
        let temp=document.createElement("p") ;
        temp.innerText=`Temperature: ${data.list[0].main.temp}°C`;
        let mxt=document.createElement("p") ;
        mxt.innerText=`Max-Temperature: ${data.list[0].main.temp_max}°C`;
        let mnt=document.createElement("p") ;
        mnt.innerText=`Min-Temperature: ${data.list[0].main.temp_min}°C`;
        ...
        snr.innerText=`SunRise: ${data.city.sunrise}`
        let sns=document.createElement("p") ;
        sns.innerText=`SunSet: ${data.city.sunset}`
